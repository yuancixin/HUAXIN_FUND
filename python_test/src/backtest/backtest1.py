...
#         print('以%s卖出' % price)
    ...
    def unwind(S, B):
        nonlocal capital_available, hold_num, B_PRICE, S_PRICE, trade_num , capital_last, retreat_max
...
#             print('======%s因合约结束，平仓======' % df_year[i:i + 1]['trade_date'].values[0])
    ...
    print('%s日均线--交易次数%s--单次最大回撤%s--最终资产%s' % (N, trade_num, round(retreat_max, 2), round(capital_available, 2)))
